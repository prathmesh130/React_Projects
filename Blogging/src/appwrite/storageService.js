...
export class Service {
    ...
    async createPost({ Title, slug, Content, FeaturedImg, status, userID, dislikeCount, likeCount }) {
        try {
            return await this.databases.createDocument(config.APPWRITE_DB_ID, config.APPWRITE_COLLECTION_ID, slug, {
                Title, Content, FeaturedImg, status, userID, dislikeCount, likeCount
            })
        } catch (error) {
            console.log(error)
        }
    ...
    async getAllPosts(quries = [Query.equal("status", "active")]) {
        try {
